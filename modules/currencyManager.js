...
class CurrencyManager {
    ...
    async createUser(userId, balance = 1000, lastDaily = null) {
        try {
            ...
            secureLogger.error("Error creating user currency", {
                error: error.message,
                userId,
            });
        }
    ...
    async getBalance(userId) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error getting balance", {
                error: error.message,
                userId,
            });
            return 1000;
    ...
    async addBalance(userId, amount) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error adding balance", {
                error: error.message,
                userId,
            });
    ...
    async subtractBalance(userId, amount) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error subtracting balance", {
                error: error.message,
                userId,
            });
            return false;
    ...
    async getDailyBonus(userId) {
        ...
        try {
            ...
            if (rows.length === 0) {
                ...
                return {
                    success: false,
                    message: "Welcome! You start with 1000 coins!",
                };
            }
            ...
            if (player.balance >= max_coins) {
                return {
                    success: false,
                    message: `You have enough coins! Daily bonus is for players with less than ${max_coins} coins.`,
                };
            }

            let bonus = Math.min(bonus_payout, max_coins - player.balance);
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error getting daily bonus", {
                error: error.message,
                userId,
            });
    ...
    async getLeaderboard(limit = 10) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error getting leaderboard", {
                error: error.message,
            });
            return [];
        }
    ...
    async awardCoins(userId, amount, reason = "Game reward") {
        try {
            ...
            const player = rows[0] ||
                rows || { balance: 1000, win_streak: 0, last_win: null };
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.info("Awarding coins to user", {
                amount,
                reason: SecurityUtils.sanitizeForLog(reason),
                userId,
            });
            ...
            if (reason.includes("win")) {
                const today = new Date().toISOString().split("T")[0];
                const lastWinDate = player.last_win
                    ? new Date(player.last_win).toISOString().split("T")[0]
                    : null;
                ...
                const secureLogger = require("./secureLogger");
                secureLogger.info("Bonus calculation", {
                    finalAmount,
                    streak: newStreak,
                    firstWin: firstWinBonus,
                    userId,
                });
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error awarding coins", {
    ...
    async resetStreak(userId) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error resetting streak", {
                error: error.message,
                userId,
            });
            throw error;
    ...
    async logTransaction(userId, amount, reason, balanceBefore, balanceAfter) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error logging transaction", {
                error: error.message,
                userId,
            });
        }
    ...
    async getTransactionHistory(userId, limit = 10) {
        try {
            ...
            const rows = await database.query(
                ...
                [userId, safeLimit.toString()]
            );
            return rows || [];
        } catch (error) {
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error getting transaction history", {
                error: error.message,
                userId,
            });
            return [];
    ...
    async spendCoins(userId, amount, reason = "Purchase") {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error spending coins", {
                error: error.message,
                userId,
            });
            return false;
    ...
    async adminAddCoins(userId, amount, reason = "Admin grant") {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error adding coins", {
                error: error.message.substring(0, 100),
                userId,
            });
    ...
    async adminRemoveCoins(userId, amount, reason = "Admin removal") {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error removing coins", {
                error: error.message.substring(0, 100),
                userId,
            });
            return { success: false, error: error.message };
    ...
    async getAllTransactions(limit = 50) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error getting all transactions", {
                error: error.message,
            });
            return [];
        }
    ...
    async activateDailyBoost(userId) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error activating daily boost", {
                error: error.message,
                userId,
            });
            return false;
    ...
    async hasDailyBoost(userId) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error checking daily boost", {
                error: error.message,
                userId,
            });
    ...
    async addStreakShield(userId, count = 1) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error adding streak shield", {
                error: error.message,
                userId,
            });
    ...
    async hasStreakShield(userId) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error checking streak shield", {
                error: error.message,
                userId,
            });
            return false;
    ...
    async recordLoss(userId) {
        try {
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error recording loss", {
                error: error.message,
                userId,
            });
            return { shieldUsed: false };
    ...
    async getBoostStatus(userId) {
        try {
            ...
            const user = rows[0];
            const dailyBoost =
                user.daily_boost_expires &&
                new Date(user.daily_boost_expires) > new Date();
            ...
            const secureLogger = require("./secureLogger");
            secureLogger.error("Error getting boost status", {
                error: error.message,
                userId,
            });
            return { dailyBoost: false, streakShields: 0 };

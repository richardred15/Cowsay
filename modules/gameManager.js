...
class GameManager {
    constructor() {
        ...
        this.games = {
            ...
            unoexpress: require("./games/unoexpress"),
    ...
    createGamesEmbed() {
        ...
        return new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: "📝 Wordle",
                    value: "`!wordle [bet]` - 5-letter word guessing game\n• **6 guesses**: Standard Wordle rules\n• **Betting system**: Optional coin wager (2x payout)\n• **Text input**: `!guess HOUSE` to make guesses",
                    inline: false,
                },
                {
                    name: "🎯 Other Commands",
            ...
            .setFooter({
                text: "Use coins to bet in blackjack! Check !cowsay balance",
            });
    }
    ...
    createBlackjackHelpEmbed() {
        ...
        return new EmbedBuilder()
            ...
            .setFooter({
                text: "Minimum bet: 10 coins • Use !cowsay daily for free coins!",
            });
    }
    ...
    async startBlackjackGame(message, mode, betAmount) {
        ...
        if (mode === "single") {
            return await game.startSinglePlayer(
                fakeInteraction,
                this,
                betAmount
            );
            ...
            return await game.showWaitTimeSelection(
                fakeInteraction,
                isDealer,
                betAmount
            );
            ...
            const fakeInteraction = {
                ...
                update: async (options) => {
                    try {
                        ...
                        Logger.error(
                            "Failed to update message in fake interaction",
                            error.message
                        );
    ...
    async handleButtonInteraction(interaction) {
        ...
        const isGameButton =
            gameButtonPrefixes.some((prefix) =>
                interaction.customId.startsWith(prefix)
            ) ||
            ["bj_join", "bj_cancel", "bj_start", "bj_game_view"].includes(
        ...
        if (gameType === "roulette") {
            ...
            for (const [key, data] of this.activeGames) {
                if (
                    ...
                    return await game.handleInteraction(
                        interaction,
                        data,
                        key,
                        this
        ...
        if (gameType === "baccarat") {
            ...
            await interaction.reply({
                content: "No active baccarat game found!",
                flags: require("discord.js").MessageFlags.Ephemeral,
            });
            return true;
        }
        ...
        if (gameType === "unoexpress") {
            ...
            await interaction.reply({
                content: "No active UNO Express game found!",
                flags: require("discord.js").MessageFlags.Ephemeral,
            });
            return true;
        }
        ...
        if (
            ["bj_join", "bj_cancel", "bj_start"].includes(
                interaction.customId
            ) ||
        ...
        if (interaction.customId === "bj_game_view") {
            ...
            for (const [key, data] of this.activeGames) {
                if (
                    data &&
                    data.type === "blackjack" &&
                    (data.channelId === channelId || key.includes(channelId))
                ) {
                    return await game.handleInteraction(
                        interaction,
                        data,
    ...
    getGameTypeFromInteraction(interaction) {
        ...
        if (customId.startsWith("wordle_")) return "wordle";
        return null;
    }
    ...
    findUserGame(userId, gameType) {
        ...
        for (const [key, data] of this.activeGames) {
            ...
            if (data.type === "blackjack") {
                if (
                    (data.phase === "setup" && data.creator === userId) ||

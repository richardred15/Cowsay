...
class GiftManager {
    async sendGift(senderId, recipientId, itemId, message = null) {
        try {
            ...
            if (senderId === recipientId) {
                return { success: false, error: "You can't gift items to yourself!" };
            }
            ...
            const [recipientOwns, item, senderBalance] = await Promise.all([
                ...
                database.query('SELECT item_id, name, price, category FROM shop_items WHERE item_id = ?', [itemId]),
            ...
            if (senderBalance < giftCost) {
                return { success: false, error: `Insufficient coins! Need ${giftCost} coins (${basePrice} + 10% gift fee).` };
            }
            ...
            try {
                ...
                await currencyManager.removeCoins(senderId, giftCost, `Gift: ${item[0].name} to user`);
                
                // Add item to recipient's inventory
                await inventoryManager.addToInventory(recipientId, itemId, 'gift', senderId);
                ...
                Logger.info(`Gift sent: ${senderId} -> ${recipientId}, item: ${itemId}, cost: ${giftCost}`);
                return { 
                    success: true, 
                    item: item[0], 
                    cost: giftCost,
    ...
    async addToWishlist(userId, itemId, message = null) {
        try {
            ...
            const item = await database.query('SELECT * FROM shop_items WHERE item_id = ?', [itemId]);
            if (item.length === 0) {
                return { success: false, error: "Item not found!" };
            }

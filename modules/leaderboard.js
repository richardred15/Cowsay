...
async function handleLeaderboardResponse(message, llmProvider) {
    ...
    if (
        Date.now() - leaderboardData.timestamp < 30000 &&
        (message.content.includes("Leaderboard") ||
            message.content.includes("Level") ||
        ...
        try {
            ...
            leaderboardText = SecurityUtils.sanitizeForDisplay(
                memberCache.replaceUsernamesWithDisplayNames(
                    leaderboardText,
                    message.guild
                )
            );
            ...
            contextManager.addLeaderboardSnapshot(
                message.guild.id,
                leaderboardText
            );
            const changes = contextManager.getLeaderboardChanges(
                message.guild.id
            );
            const contextSummary = changes
                ? `Changes: ${changes
                      .map((change) => {
                          if (change.isNew) {
                          ...
                          if (change.rankChange > 0)
                              desc += ` (↑${change.rankChange})`;
                          if (change.rankChange < 0)
                              desc += ` (↓${Math.abs(change.rankChange)})`;
                          if (change.xpGain > 0)
                              desc += ` +${change.xpGain} XP`;

                          return desc;
                      })
                      .join(", ")}`
                : "First leaderboard check - no changes to compare.";
            ...
            const systemPrompt =
                (await getSystemPrompt(LLM_PROVIDER, message.guild?.id)) +
                " Focus on providing SHORT commentary on these leaderboard results using actual player names and scores. Celebrate winners and make friendly jokes about score gaps.";
            ...
            let commentary = await llmService.generateResponse(
                messages,
                options
            );
            // Remove quotes that LLM might add around the response
            commentary = commentary.replace(/^["']|["']$/g, "");
            Logger.debug("Generated commentary preview", {
                preview: commentary.substring(0, 100),
            });

            if (commentary && commentary.trim() !== "") {
                // Process any tags used in the commentary
                const { processedMessage, usedTags } =
                    tagManager.processTagsInMessage(commentary, availableTags);

                const maxLength = 1800;
                if (processedMessage.length > maxLength) {
                    commentary =
                        processedMessage.slice(0, maxLength) +
                        " [... Moo! ...]";
                } else {
                ...
                if (usedTags.length > 0) {
                    Logger.info("Tags used in commentary", {
                        tags: usedTags.map((t) => t.displayName),
                    });
                }
                ...
                setTimeout(async () => {
                    try {
                        Logger.debug("Sending commentary", {
                            preview: commentary.substring(0, 100),
                        });
                        ...
                        Logger.error(
                            "Failed to send leaderboard commentary",
                            error.message
                        );
                    }
                }, 2000);

...
const { createMockInteraction, createMockMessage } = require('../mocks/discord-mock');
...
describe('Roulette Integration Tests', () => {
    ...
    describe('Payout System', () => {
        test('should calculate payouts correctly for winning red bet', async () => {
            ...
            const results = await roulette.calculatePayouts(gameData, winningNumber);
        ...
        test('should handle straight number bet win', async () => {
            ...
            const gameData = {
                players: new Map([
                    [userId, {
                        name: 'StraightWinner',
                        bets: [{ type: 'straight', number: 7, amount: 10, payout: 35 }],
                        totalBet: 10
                    }]
                ])
            };
